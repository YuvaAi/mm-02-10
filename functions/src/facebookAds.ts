...
function convertCountryNamesToCodes(countryNames: string[]): string[] {
 ...
 */
export const createNewCampaignFunction = async (
  data: any,
  context: { auth?: any }
  ...
  try {
    ...
    const accessToken = adsCreds?.accessToken;
    const adAccountId = adsCreds?.adAccountId || adsCreds?.pageId; // Use pageId as fallback
    ...
    if (!campaignResponse.ok) {
      ...
      throw new Error(`Failed to create campaign: ${campaignResult.error?.message || 'Unknown error'}`);
    }
...
export const createAdSetAndAd = (
  async (data: CreateAdRequest, context: { auth?: any }): Promise<FacebookAdResponse> => {
    ...
    try {
      ...
      const FACEBOOK_CONFIG: FacebookAdsCredentials = {
        ACCESS_TOKEN: adsCreds?.accessToken || facebookCreds?.accessToken || '',
        CAMPAIGN_ID: adsCreds?.campaignId || '',
        AD_ACCOUNT_ID: adsCreds?.adAccountId || facebookCreds?.adAccountId || '',
        PAGE_ID: facebookCreds?.pageId || adsCreds?.pageId || '',
      ...
      if (data.image_url && data.caption && FACEBOOK_CONFIG.INSTAGRAM_USER_ID) {
        ...
        const instagramResult = await postToInstagram(
          accessToken,
          FACEBOOK_CONFIG.INSTAGRAM_USER_ID,
          data.image_url,
          data.caption
        );
      ...
      const targeting = { ...DEFAULT_CONFIG.targeting, ...data.targeting };
 ...
 */
export const createFacebookAdCampaign = async (data: any, context: { auth?: any }) => {
  try {
    // Verify user is authenticated
    if (!context.auth) {
      throw new Error('User must be authenticated to create ads');
    ...
    const adAccountId = credentials?.adAccountId || process.env.FACEBOOK_AD_ACCOUNT_ID;
    const pageId = credentials?.pageId || facebookCredentials?.pageId || process.env.FACEBOOK_PAGE_ID;
...
async function postToInstagram(
  ...
  try {
    ...
    if (!mediaResponse.ok) {
      ...
      return {
        success: false,
        error: `Failed to create Instagram media: ${mediaResult.error?.message || 'Unknown error'}`
      };
    }
    ...
    if (!publishResponse.ok) {
      ...
      return {
        success: false,
        error: `Failed to publish Instagram media: ${publishResult.error?.message || 'Unknown error'}`
      };
    }
...
async function createAdSet(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create ad set: ${result.error?.message || 'Unknown error'}`);
  }
...
async function createAd(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create ad: ${result.error?.message || 'Unknown error'}`);
  }
...
async function createAdCreativeStep(
  accessToken: string,
  adAccountId: string,
  adSetId: string,
  pageId: string,
  ...
  console.log('🔍 Creating ad creative with parameters:', {
    ...
    pageId,
    imageHash,
    adText: campaignData.adText?.substring(0, 50) + '...',
