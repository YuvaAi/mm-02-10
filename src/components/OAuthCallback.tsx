  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (!state || !OAuthService.validateState(state)) {
        setStatus('error');
        setMessage('Invalid state parameter. This may be a security issue.');
        return;
      }
      ...
      try {
        ...
        switch (platform) {
          case 'facebook':
            result = await OAuthCallbackHandler.handleFacebookCallback(currentUser.uid, code, state);
            break;
          case 'linkedin':
            result = await OAuthCallbackHandler.handleLinkedInCallback(currentUser.uid, code, state);
            break;
          default:
            setStatus('error');
