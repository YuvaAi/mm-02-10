...
const LinkedInOAuthDebugger: React.FC = () => {
  const [testResults, setTestResults] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(false);

  const runLinkedInOAuthTest = async () => {
    setIsLoading(true);
    setTestResults(null);
    ...
    try {
      ...
      const testResults = {
        ...
        validation: {
          ...
          scopeValid: scope.includes('w_member_social')
        },
        generatedUrl: {
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-900">LinkedIn OAuth Debugger</h2>
          <button
            onClick={runLinkedInOAuthTest}
            disabled={isLoading}
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <code className="bg-gray-200 px-2 py-1 rounded text-sm">
                      {testResults.environment?.clientId || 'NOT SET'}
                    </code>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <code className="bg-gray-200 px-2 py-1 rounded text-sm">
                      {testResults.environment?.clientSecret || 'NOT SET'}
                    </code>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <code className="bg-gray-200 px-2 py-1 rounded text-sm">
                      {testResults.environment?.scope}
                    </code>
                    {testResults.validation?.scopeValid ? (
                      <CheckCircle className="w-4 h-4 text-green-500" />
                    ) : (
                      <AlertCircle className="w-4 h-4 text-red-500" />
                    )}
        ...
        <div className="mt-8 bg-yellow-50 rounded-lg p-4">
          ...
          <ol className="list-decimal list-inside space-y-2 text-yellow-700">
            <li>Ensure your LinkedIn app is configured with the correct redirect URI</li>
            <li>Verify that your LinkedIn app has the required permissions</li>
            <li>Check that your LinkedIn app is in "Live" mode (not development)</li>
            <li>Make sure the redirect URI matches exactly: <code>{window.location.origin}/oauth/linkedin/callback</code></li>
            <li>Test the generated OAuth URL by clicking the "Open in new tab" button</li>
          </ol>
        </div>
      </div>
    </div>
...
export default LinkedInOAuthDebugger;
